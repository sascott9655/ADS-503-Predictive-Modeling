...
I would choose the Kappa statistic to optimize as it is great at dealing with class inbalance and specifically used for two class classification. Because our data set has either a 'Yes' or 'No' response, the Kappa statistics would be valuable to optimize due to this data set being two class classification and that the class balance is not exactly even.
...
```{r 5_1_c_bio}
#Getting rid of nearZeroVar #82 were removed
low_freq_bio <- nearZeroVar(bio)
bio <- bio[,-low_freq_bio]

# removing high correlation variables #21 were removed
bio_cor <- cor(bio)
high_bio_cor <- findCorrelation(bio_cor, cutoff = .75)
#length(high_bio_cor)
bio <- bio[, -high_bio_cor]
#corrplot::corrplot(cor(bio))
...
```{r}
set.seed(rseed)
log_reg <- train(train_bio, y_train,
                 method = "glm",
                 metric = "ROC",
                 trControl = ctrl)
log_reg
